...
class ProductSummary(BaseModel):
    ...
    product_type: str = Field(
        ..., description="Type of insurance (e.g., 'Seguro de Hogar', 'Seguro de Salud')"
    )
    target_audience: str | None = Field(
        None, description="Target market (e.g., 'Autónomos', 'PYMES', 'Particulares')"
    )
    ...
    key_benefits: list[str] = Field(
        default_factory=list, description="Main selling points and benefits"
    )
    insurance_company: str = Field(..., description="Name of the insurance company")
    administrative_code: str | None = Field(
        None, description="DGSFP administrative code (e.g., 'C-0737')"
    )
...
class Coverage(BaseModel):
    ...
    benefits: list[str] = Field(
        default_factory=list, description="Benefits or compensation provided"
    )
    capital_or_limit: str | None = Field(
        None, description="Maximum coverage amount (e.g., '100.000€', 'unlimited')"
    )
    ...
    conditions: list[str] | None = Field(
        None, description="Special conditions or requirements for this coverage"
    )
...
class Exclusion(BaseModel):
    ...
    category: str | None = Field(
        None, description="Category of exclusion (e.g., 'General', 'Specific to coverage X')"
    )
    legal_reference: str | None = Field(
        None, description="Legal article or clause reference if available"
    )
...
class LimitOrRestriction(BaseModel):
    ...
    limit_value: str = Field(
        ..., description="Value of the limit (e.g., '12.000€', '15 sessions', '90 days')"
    )
...
class ContractTerms(BaseModel):
    ...
    duration: str = Field(
        ..., description="Contract duration (e.g., 'Annual', 'Annual with automatic renewal')"
    )
    renewal_type: str = Field(
        ..., description="Type of renewal (e.g., 'automatic', 'manual')"
    )
    ...
    termination_causes: list[str] = Field(
        default_factory=list, description="Causes that can terminate the contract"
    )
    payment_method: str | None = Field(
        None, description="Payment method (e.g., 'Bank direct debit')"
    )
...
class PricingInfo(BaseModel):
    ...
    pricing_factors: list[str] = Field(
        default_factory=list, description="Factors that affect the premium price"
    )
...
class GeographicCoverage(BaseModel):
    ...
    primary_territory: str = Field(
        ..., description="Primary coverage territory (e.g., 'Spain', 'European Union')"
    )
    worldwide_coverage: list[str] | None = Field(
        None, description="List of coverages that apply worldwide"
    )
    special_conditions_abroad: str | None = Field(
        None, description="Special conditions for coverage abroad"
    )
...
class SourceDocument(BaseModel):
    ...
    extraction_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score of extraction (0.0 to 1.0)"
    )
...
class ProductAnalysis(BaseModel):
    ...
    coverages: list[Coverage] = Field(
        default_factory=list, description="List of coverages/guarantees offered"
    )

    # What's NOT covered
    exclusions: list[Exclusion] = Field(
        default_factory=list, description="List of exclusions (what is not covered)"
    )
    ...
    obligations: PolicyholderObligations = Field(
        ..., description="Policyholder duties and obligations"
    )

    # Contract terms
    contract_terms: ContractTerms = Field(
        ..., description="Contract duration and termination terms"
    )

    # Pricing info (when available)
    pricing: PricingInfo | None = Field(
        None, description="Pricing information (may not be available in all documents)"
    )

    # Geographic scope
    geographic_coverage: GeographicCoverage = Field(
        ..., description="Geographic scope of coverage"
    )
...
class InsuranceProduct(BaseModel):
    ...
    analysis: ProductAnalysis | None = Field(
        None, description="Structured analysis extracted from policy documents"
    )
